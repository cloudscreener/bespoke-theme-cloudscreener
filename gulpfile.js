var os = require('os'),
  ...
  browser = os.platform() === 'linux' ? 'chromium' : (
...
gulp.task('browserify:lib', ['clean:browserify:lib', 'stylus'], function() {
  return gulp.src('lib/bespoke-theme-cloudscreener.js')
    .pipe(isDemo ? plumber() : through())
    .pipe(browserify({ transform: ['brfs'], standalone: 'bespoke.themes.cloudscreener' }))
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-theme-cloudscreener.min.js'))
    .pipe(uglify())
    .pipe(header(template([
      '/*! <%= name %> v<%= version %> ',
...
gulp.task('connect', ['compile'], function(done) {
  ...
  opn('http://localhost:8080', browser);
});
